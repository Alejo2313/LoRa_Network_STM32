...
#define DELAY_TIME	100/portTICK_RATE_MS
...
#else
	Trace_send("%X %X %X %X %X ", data->NetAddr, data->devAddr, data->devDest, data->MacType, data->pSize);
#endif
...
void processJoin(Packet_t *inData)
{
	...
	*(TxData->pData + cnt++) = RX_TIMEOUT_VALUE / 1000;
	...
	vTaskDelayUntil(&tmp, 100/portTICK_RATE_MS );
}
...
void waitForPacket()
{
	...
	while (!Trace_dataAvailable())
	{
		if(xTaskGetTickCount() - tmp > DELAY_TIME)
		{
			return;
		}
	}
